import type { ExtractedMedicine } from '../../utils/parseExtractedText'
...
function mergeWithFallback(
  base: ExtractedMedicine,
  incoming: Partial<ExtractedMedicine>,
): ExtractedMedicine {
  return {
...
function buildPrompt(ocrText: string, base: ExtractedMedicine) {
  return `
...
type GeminiChatMessage = { role: 'user' | 'assistant'; content: string }
...
async function enrichWithGemini(
  apiKey: string,
  prompt: string,
): Promise<Partial<ExtractedMedicine>> {
  ...
  if (!resp.ok) {
    ...
    throw new Error(
      `Gemini API failed (${resp.status}) model=${model} body=${errorBody}`,
    )
  }
      ...
      messages: [
        {
          role: 'system',
          content:
            'You extract medicine data from OCR text. Return only JSON object with requested fields.',
        },
...
export async function enrichMedicineWithLLM(
  ocrText: string,
  fallbackParsed: ExtractedMedicine,
): Promise<{ enriched: ExtractedMedicine; provider: string; profileMarkdown: string }> {
  ...
  if (provider === 'gemini') {
    ...
    if (!key) throw new Error('AI_PROVIDER is gemini but GEMINI_API_KEY is missing')
    const llmData = await withRetry(() => enrichWithGemini(key, prompt))
    return {
      enriched: mergeWithFallback(fallbackParsed, llmData),
      provider: 'gemini',
      profileMarkdown: (llmData as any)?.profileMarkdown ?? '',
    }
  }

  const key = process.env.OPENAI_API_KEY ?? ''
  if (!key) throw new Error('AI_PROVIDER is openai but OPENAI_API_KEY is missing')
...
export async function chatEnrichWithLLM(
  ocrText: string,
  current: ExtractedMedicine,
  messages: GeminiChatMessage[],
): Promise<{
  enriched: ExtractedMedicine
  provider: string
  profileMarkdown: string
}> {
  const provider = getProvider()
...
type InsightsJson = { markdown: string }
...
async function insightsWithGemini(apiKey: string, prompt: string): Promise<InsightsJson> {
  const model = process.env.GEMINI_MODEL ?? 'gemini-2.0-flash'
  const resp = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(
      model,
  ...
  return JSON.parse(cleanJsonText(text)) as InsightsJson
}

async function insightsWithOpenAI(apiKey: string, prompt: string): Promise<InsightsJson> {
  const model = process.env.OPENAI_MODEL ?? 'gpt-4o-mini'
  const resp = await fetch('https://api.openai.com/v1/chat/completions', {
    method: 'POST',
    headers: {
